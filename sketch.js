...
function draw() {
  ...
  if (isOnGround2) {
    ...
    if (isPlayer1Near) {
      ...
      // --- 傷害與擊退邏輯 ---
      // 在攻擊動畫的特定影格（例如第5個影格，索引為4）且角色1可被攻擊時觸發
      if (currentFrame2 === 4 && canBeHit) {
        let knockbackForce = 60; // 擊退的力道
        // 根據角色2的方向決定擊退方向
        charX += direction2 * knockbackForce;
        canBeHit = false; // 確保在同一次攻擊中只被擊退一次
      }
    } else if (isMoving2) {
      // 走路動畫 (使用全部6個影格)
  ...
  // --- 角色 1 對話框邏輯 ---
  // 當角色1靠近且有輸入文字時顯示對話框
  if (isPlayer1Near && char1Speech.length > 0) {
    push();
    let speech = char1Speech;
    // 計算角色1的顯示高度，用於定位對話框
    let char1DisplayedHeight = spriteSheet.height * scaleFactor;
    let textBubbleY1 = charY - (char1DisplayedHeight / 2) - 60; // 在角色頭頂上方
    ...
    stroke(0); // 黑色邊框
    strokeWeight(2);
    rectMode(CENTER); // 將矩形的繪製基準點設為中心
    rect(charX, textBubbleY1, boxWidth, boxHeight, 10); // 圓角矩形

    // 繪製文字
    noStroke();
    fill(0); // 黑色文字
    text(speech, charX, textBubbleY1);
    pop();
  }
  ...
  if (isPlayer1Near && isOnGround2) {
    push();
    let speech = "你叫什麼名字";
    let textBubbleY = char2Y - (sHeight2 * scaleFactor2 / 2) - 60; // 在角色頭頂上方
    ...
    strokeWeight(2);
    rectMode(CENTER); // 將矩形的繪製基準點設為中心
    rect(char2X, textBubbleY, boxWidth, boxHeight, 10);

    // 繪製文字
    noStroke();
    fill(0); // 黑色文字
    text(speech, char2X, textBubbleY);
    pop();
  }
}
